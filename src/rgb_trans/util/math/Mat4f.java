...
public class Mat4f {
	...
	//s[row][column]
		...
		public Mat4f translate(Vec3f delta){
			...
			return Mat4f.mul(translation, this);
		}
		
		public Mat4f shear(float shyx, float shzx, float shxy, float shzy, float shxz, float shyz){
			Mat4f shear = new Mat4f();
			shear.s01 = shyx;
			shear.s02 = shzx;
			shear.s10 = shxy;
			shear.s12 = shzy;
		...
		public static Mat4f getTransformationMatrix(Vec3f translation, Vec3f rotation, Vec3f scale){
			return new Mat4f().scale(scale).rotate(rotation).translate(translation);
		}
		...
		// End of 3D transformation matrices
